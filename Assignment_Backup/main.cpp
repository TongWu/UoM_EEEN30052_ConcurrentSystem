...
double sleepRandomPeriod1();
void printCompetitor(Competitor& c, double sleep_time);
void run(Competitor &c, SyncAgent& s, int o, int p);
void done();
void isDone();
void find_winner();
...
int sleepRandomPeriod()  {
	std::uniform_int_distribution<> dis(9, 15); //generate a random integer between 1-10.
	int n = dis(gen);
	return n;
}

double sleepRandomPeriod1()  {
	std::uniform_real_distribution<> dis(9, 15); //generate a random integer between 1-10.
	double n = dis(gen);
	return n;
}
...
void run(Competitor &c, SyncAgent& s, int o, int p)
{
	...
	double sleep_time = sleepRandomPeriod1();
	// Pause each thread here
	s.pause();
	// Delay for random period
	std::this_thread::sleep_for(std::chrono::seconds(long(sleep_time)));
	std::unique_lock<std::mutex> printLocker(print_mu);
...
int main()
{
    ...
    string person_number[4][4] = {
			{"Weilian Jiankang",   "Pide Lv"  ,   "Wujigong"  ,"Laimu Mashi"},   // Team Soft
    ...
    for (i = 0; i < NO_TEAMS; i++)
    {
        for (j = 0; j < NO_MEMBERS; j++)
        {
			...
			{theThreads[i][j] = std::thread(&run, std::ref(teamsAndMembers[i][j]), std::ref(startAgent), std::ref(i), std::ref(j));}
			else
			// Other competitors who should be located at exchange zones, pass exchange to run function
			{theThreads[i][j] = std::thread(&run, std::ref(teamsAndMembers[i][j]), std::ref(exchanges[i][j-1]), std::ref(i), std::ref(j));}
			/*
