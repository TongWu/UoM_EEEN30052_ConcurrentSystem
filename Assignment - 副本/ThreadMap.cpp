...
Competitor ThreadMap::getCompetitor (std::thread::id id) {
	std::map <std::thread::id, Competitor>::iterator itr = threadComp.find(id);
	if (itr == threadComp.end())
		return Competitor::makeNull();						//	alternatively	return *(new Competitor(" ", " "));
	else
		return itr->second;									// i.e. the second item in the pair
}